...
import model.SceneModelCode;
import model.SceneModelXML;

@:meta(SWF(width="800",height="600",backgroundColor="0x002135",frameRate="60"))
class Lava extends Sprite
{
	...
	{
		var operation : Cadet2DStartUpOperation = cast((event.target), Cadet2DStartUpOperation);  
		...
		if (_cadetFileURL != null) {
			_sceneModel = new SceneModelXML();
			_sceneModel.cadetScene = cast((operation.getResult()), CadetScene);
		} else {
			_sceneModel = new SceneModelCode(CoreApp.resourceManager);  
