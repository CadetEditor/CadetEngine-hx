...
class Boxes extends Sprite
{
	...
	public function new()
	{
		...
		// Create cubes  
		var sceneWidth : Int = 2000;
		var sceneHeight : Int = 6000;
		var sceneDepth : Int = 2000;
		var cubeSize : Int = 80;
		var variation : Int = 20;
		for (i in 0...150) {
			var x : Float = Math.random() * sceneWidth - sceneWidth / 2;
			var y : Float = Math.random() * sceneHeight;
			var z : Float = Math.random() * sceneDepth - sceneDepth / 2;
			var width : Float = cubeSize + Math.random() * variation;
			var height : Float = cubeSize + Math.random() * variation;
			var depth : Float = cubeSize + Math.random() * variation;addCubeEntity(x, y, z, width, height, depth);
		}  
	...
	private function addCubeEntity(x : Float, y : Float, z : Float, width : Float, height : Float, depth : Float) : MeshComponent
	{
