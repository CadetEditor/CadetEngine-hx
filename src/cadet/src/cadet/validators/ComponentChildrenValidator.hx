...
class ComponentChildrenValidator extends CollectionValidator
{
	...
	*/  
	...
	override public function getValidItems() : Array<Dynamic>
	{
		if (!_collection) return [];
		var validItems : Array<Dynamic> = [];  //ArrayUtil.filterByType(_collection.source, _validType);  
		
		for (i in 0..._collection.length) {
			if (Std.is(_collection[i], _validType)) {
				validItems.push(_collection[i]);
            }
			if (Std.is(_collection[i], IComponentContainer)) {
	...
	private function recurseItems(parent : IComponentContainer, validItems : Array<Dynamic>) : Array<Dynamic>
	{
		var newItems : Array<Dynamic> = ArrayUtil.filterByType(parent.children.source, _validType);
		if (newItems.length > 0) {
			validItems = validItems.concat(newItems);
        }
		for (i in 0...parent.children.length) {
			var child : IComponent = parent.children[i];
			if (Std.is(child, IComponentContainer)) {
				recurseItems(cast((child), IComponentContainer), validItems);
            }
        }
		return validItems;
    }
