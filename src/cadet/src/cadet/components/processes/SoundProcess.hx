...
class SoundProcess extends Component implements IInitialisableComponent
{
    public var muted(get, set) : Bool;
    public var music(get, set) : SoundComponent;
	private inline var SOUNDS : String = "sounds";
	private var _muted : Bool = false;
	private var _musicPlaying : Bool = false;
	private var _initialised : Bool = false;
	private var _music : SoundComponent;
	private var soundArray : Array<Dynamic>;
	
	public function new(name : String = "SoundProcess")
    {
		soundArray = new Array<Dynamic>();
		super(name);
    }
	...
	override private function addedToScene() : Void
	{
		...
		scene.addEventListener(ComponentEvent.REMOVED_FROM_SCENE, componentRemovedFromSceneHandler);addSounds();
    }  
	...
	{
		_initialised = true;invalidate(SOUNDS);
    }  
	...
	private function set_Music(value : SoundComponent) : SoundComponent
	{
		_music = value;
        return value;
    }
	
	private function get_Music() : SoundComponent
	{
		return _music;
    }  
	...
	private function validateSounds() : Void
	{
		...
		if (!_muted) {
			...
			if (_music != null && _musicPlaying) {
				_musicPlaying = false;_music.stop();
            }
        }
    }
	...
	{
		var allSounds : Array<IComponent> = ComponentUtil.getChildrenOfType(scene, ISound, true);
	...
	{
		if (Std.is(event.component, ISound) == false) return;
		addSound(cast((event.component), ISound));
    }
	
	private function componentRemovedFromSceneHandler(event : ComponentEvent) : Void
	{
		if (Std.is(event.component, ISound) == false) return;
		removeSound(cast((event.component), ISound));
    }
	
	private function addSound(sound : ISound) : Void
	{
		var i : Int = Lambda.indexOf(soundArray, sound);
