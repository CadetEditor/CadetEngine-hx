...
class InputProcess extends ComponentContainer
{
	...
	private var inputTable : Dynamic;
	...
	public function isInputDown(name : String) : Bool
	{
		return Reflect.field(inputTable, name);
    }
	...
	private function getMappingForKeyCode(keyCode : Int) : IInputMapping
	{
		for (child/* AS3HX WARNING could not determine type for var: child exp: EIdent(_children) type: null */ in _children) {
			if (Std.is(child, KeyboardInputMapping)) {
				var kim : KeyboardInputMapping = cast((child), KeyboardInputMapping);
	...
	private function getMappingForInput(input : String) : IInputMapping
	{
		for (child/* AS3HX WARNING could not determine type for var: child exp: EIdent(_children) type: null */ in _children) {
			if (Std.is(child, IInputMapping) == false) continue;
			if (cast((child), IInputMapping).input == input) return cast((child), IInputMapping);
        }
		return null;
    }
	
	private function clearTouches() : Void
	{
		for (mapping/* AS3HX WARNING could not determine type for var: mapping exp: EField(EIdent(TouchInputMapping),mappings) type: null */ in TouchInputMapping.mappings) {
