...
class GetTemplateOperation extends EventDispatcher implements IAsynchronousOperation
{
	...
	private var sceneTable : Dynamic;
	private var result : IComponent;
	
	public function new(templatePath : String, fileSystemProvider : IFileSystemProvider, domTable : Dynamic = null)
    {
        super();
	...
	public function execute() : Void
	{
		...
		if (templatePath.indexOf("#") == -1) {
			...
			var split : Array<Dynamic> = templatePath.split("#");
			uri = new URI(split[0]); 
			sceneTableID = uri.path;
			templateID = split[1];
        }
		if (Reflect.field(sceneTable, sceneTableID)) {
			getTemplateFromScene(Reflect.field(sceneTable, sceneTableID));
        } else if (uri != null) {
	...
	{
		var operation : ReadFileAndDeserializeOperation = cast((event.target), ReadFileAndDeserializeOperation);
		var scene : ICadetScene;
		try {
			scene = cast((operation.getResult()), ICadetScene);
        } catch (e : Error) {
	...
	public function getResult() : IComponent
	{
		return result;
    }
}