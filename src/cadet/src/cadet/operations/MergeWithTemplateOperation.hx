...
class MergeWithTemplateOperation extends EventDispatcher implements IUndoableOperation implements IAsynchronousOperation
{
    public var label(get, never) : String;
	private var component : IComponent;
	private var template : IComponent;
	private var clone : Bool;
	private var operation : UndoableCompoundOperation;
	
	public function new(component : IComponent, template : IComponent, clone : Bool = false)
    {
        super();
	...
	{
		var cloneOperation : CloneOperation = cast((event.target), CloneOperation);
		template = cast((cloneOperation.getResult()), IComponent);
		performMerge();
    }
	...
	private function performMerge() : Void
	{
		...
		if (Std.is(template, IComponentContainer) && Std.is(component, IComponentContainer)) {
			var templateAsContainer : IComponentContainer = cast((template), IComponentContainer);
			var componentAsContainer : IComponentContainer = cast((component), IComponentContainer);  
			// Now loop through the new component's children and handle any marked with [Cadet( inheritFromTemplate='false' )]  
			for (i in 0...templateAsContainer.children.length) {
				...
				if (inheritFromTemplateValue == null) {
					continue;
                }
	...
	{
		operation.undo();AsynchronousUtil.dispatchLater(this, new Event(Event.COMPLETE));
    }
