...
class ReadCadetFileAndDeserializeOperation extends EventDispatcher implements IAsynchronousOperation
{
	...
	public function new(uri : URI, fileSystemProvider : IFileSystemProvider, resourceManager : ResourceManager)
    {
        super();
		this.uri = uri;
	...
	public function execute() : Void
	{
		...
		readFileOperation.addEventListener(OperationProgressEvent.PROGRESS, readFileProgressHandler); readFileOperation.addEventListener(ErrorEvent.ERROR, passThroughHandler);
	...
	{
		var deserializeOperation : DeserializeOperation = cast((event.target), DeserializeOperation);
		
		try {
			scene = cast((deserializeOperation.getResult()), ICadetScene);
        } catch (e : Error) {
			dispatchEvent(new ErrorEvent(ErrorEvent.ERROR, false, false, e.message));
			return;
        }
	...
	{
		return "Read Cadet File : " + uri.path;
    }
}