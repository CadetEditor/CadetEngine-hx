...
class CadetScene extends ComponentContainer implements ICadetScene
{
	...
	private var _runMode : Bool = false;
	private var lastFrameTime : Int = Math.round(haxe.Timer.stamp() * 1000);
	private var steppableComponents : Array<IComponent>;
	private var initOnRunComponents : Array<IComponent>;
	private var _dependencyManager : DependencyManager;
	...
	public function new()
    {
		...
		name = "Scene";
		_scene = this;
		steppableComponents = new Array<IComponent>();
		initOnRunComponents = new Array<IComponent>();
	...
	private function componentAddedToSceneHandler(event : ComponentEvent) : Void
	{
		...
		var onAList : Bool = false;
		if (Std.is(component, ISteppableComponent)) {
			steppableComponents.push(component);onAList = true;
        }
		if (Std.is(component, IInitialisableComponent)) {
			initOnRunComponents.push(component);onAList = true;  
			// if scene already running, init the component  
			if (runMode) {
				cast((component), IInitialisableComponent).init();
            }
        }
		if (onAList) {
	...
	{
		var component : IComponent = event.component;
		var steppableIndex : Int = Lambda.indexOf(steppableComponents, component);
	...
	{
		if (!runMode) {
			for (iORComponent in initOnRunComponents) {
				iORComponent.init();
            }
			runMode = true;
        }
		
		var timeStep : Float = 1 / _framerate;
		var currentTime : Int = Math.round(haxe.Timer.stamp() * 1000);
		var elapsed : Int = currentTime - lastFrameTime;
		if (elapsed < (timeStep * 1000)) {
			return;
        }
		var dt : Float = timeStep * timeScale;
		for (steppableComponent in steppableComponents) {
	...
	override private function childAdded(child : IComponent, index : Int) : Void
	{
	...
	@:meta(Serializable())
	private function set_DependencyManager(value : DependencyManager) : DependencyManager
	{
		_dependencyManager = value;
        return value;
    }
	
	private function get_DependencyManager() : DependencyManager
	{
		return _dependencyManager;
    }
	
	@:meta(Serializable(type="rawObject"))
	private function set_UserData(value : Dynamic) : Dynamic
	{
		_userData = value;
        return value;
    }
	...
	private function get_RunMode() : Bool
	{
		return _runMode;
    }
	
	private function set_RunMode(value : Bool) : Bool
	{
		_runMode = value;
        return value;
    }  
