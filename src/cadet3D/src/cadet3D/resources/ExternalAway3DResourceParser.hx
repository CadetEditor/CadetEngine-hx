...
// All rights reserved.  package cadet3d.resources;
...
class ExternalAway3DResourceParser implements IExternalResourceParser
{
	private var idPrefix : String;
	private var resourceArray : Array<Dynamic>;
	...
	private function assetCompleteHandler(event : AssetEvent) : Void
	{
		...
		var _sw0_ = (event.asset.assetType);		

		switch (_sw0_)
		{
			case AssetType.GEOMETRY:
				resource = new Away3DGeometryResource(id, cast((event.asset), Geometry));
			case AssetType.MATERIAL:
				resource = new Away3DMaterialResource(id, cast((event.asset), MaterialBase));
			case AssetType.MESH:resource = new Away3DMeshResource(id, cast((event.asset), Mesh));
		}
	...
	{
		var loader : Loader3D = cast((event.target), Loader3D);
		var resource : Away3DContainer3DResource = new Away3DContainer3DResource(idPrefix, loader);
		resourceArray.push(resource); resourceManager.addResource(resource);
