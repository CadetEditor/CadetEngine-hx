...
class SceneParser
{
	...
	private static var _meshes : Array<Dynamic>;
	private static var _wireframes : Array<Dynamic>;
	private static var _segmentSets : Array<Dynamic>;
	private static var _containers : Array<Dynamic>;
	private static var _cameras : Array<Dynamic>;  
	// Geometries, materials, lights and segments are shared resources  
	private static var _geometries : Dictionary;
	private static var _materials : Dictionary;
	private static var _lights : Dictionary;
	private static var _segments : Dictionary;
	private static var _numContainers : Int;
	private static var _numMeshes : Int;
	private static var _numWireframes : Int;
	private static var _numSegments : Int;
	private static var _numSegmentSets : Int;
	private static var _oldNumSegments : Int;
	private static var _segmentId : Int;
	private static var _numGeometries : Int;
	private static var _numMaterials : Int;
	private static var _numLights : Int;
	private static var _numCameras : Int;
	private static var _types : Dictionary;
	private static var _rootObject : Object3D;  
	// feedback  
	private static var _messages : Array<Dynamic>;
	private static var _defaultWireframeType : Class<Dynamic> = ObjectContainer3D;
	
	public function new()
	{
	}
	
	private static function get_containers() : Array<Dynamic>
	{
		return _containers;
	}
	
	private static function get_wireframes() : Array<Dynamic>
	{
		return _wireframes;
	}
	
	private static function get_meshes() : Array<Dynamic>
	{
		return _meshes;
	}
	
	private static function get_segmentSets() : Array<Dynamic>
	{
		return _segmentSets;
	}
	
	private static function get_cameras() : Array<Dynamic>
	{
		return _cameras;
	}
	
	private static function get_geometries() : Array<Dynamic>
	{
		return convertDictionaryToArray(_geometries);
	}
	
	private static function get_lights() : Array<Dynamic>
	{
		return convertDictionaryToArray(_lights);
	}
	
	private static function get_materials() : Array<Dynamic>
	{
		return convertDictionaryToArray(_materials);
	}
	
	private static function get_segments() : Array<Dynamic>
	{
		return convertDictionaryToArray(_segments);
	}
	
	private static function get_messages() : Array<Dynamic>
	{
		return _messages;
	}
	
	private static function get_numSegmentSets() : Int
	{
		return _numSegmentSets;
	}
	...
	public static function parse(object3d : Object3D) : Void
	{
		init();
	...
	private static function parseInternal(object3d : Object3D, containerId : Int = -1) : Void
	{
		if (Std.is(object3d, Mesh)) {
			var obj : SerializedObject = getObjectData(object3d, _numMeshes++, containerId);
			_meshes.push(obj);
		} else if (Std.is(object3d, LightBase)) {
			if (!Reflect.field(_lights, Std.string(object3d))) {
				obj = getObjectData(object3d, _numLights++, containerId);
				Reflect.setField(_lights, Std.string(object3d), obj);
			}
		} else if (Std.is(object3d, Camera3D)) {
			obj = getObjectData(object3d, _numCameras++, containerId);
			_cameras.push(obj);
		} else if (Std.is(object3d, WireframePrimitiveBase)) {
		...
		} else if (Std.is(object3d, SegmentSet)) {
			obj = getObjectData(object3d, _numSegmentSets, containerId);
			_numSegmentSets++;
			_segmentSets.push(obj);
		} else {
			var container : ObjectContainer3D = (try cast(object3d, ObjectContainer3D) catch (e:Dynamic) null);
	...
	public static function getObjectData(object : Dynamic, uniqueId : Int = 0, containerId : Int = -1, type : Class<Dynamic> = null) : SerializedObject
	{
		if (_types == null) init();
		var useTypeProvided : Bool = false;
		if (type == null) {
			useTypeProvided = true;
			type = getType(object);
		}
		var parseFunc : Function = Reflect.field(_types, Std.string(type));
	...
	//===== PARSE GEOMETRY =======================================================  
	private static function getConeGeometryData(cone : ConeGeometry, obj : SerializedObject) : SerializedObject
	{  
	...
	{  
		...
		var radius : Float = regPoly.radius;
		if (radius == 0) radius = regPoly.topRadius;
		obj.constructorArgs.push(new NameValuePair("radius", mathRound(radius, 2)), new NameValuePair("sides", regPoly.sides), new NameValuePair("yUp", regPoly.yUp));
		return obj;
	}
	...
	*/    
	...
	//===========================================================================    
	//===== PARSE MATERIALS =====================================================  
	private static function getColorMaterialData(colorMaterial : ColorMaterial, obj : SerializedObject) : SerializedObject
	{
		obj.constructorArgs.push(new NameValuePair("color", "0x" + Std.string(colorMaterial.color)), new NameValuePair("alpha", colorMaterial.alpha));
		return obj;
	}  
	
	//===========================================================================    
	//===== PARSE LIGHTS ========================================================  
	private static function getDirectionalLightData(dirLight : DirectionalLight, obj : SerializedObject) : SerializedObject
	{
		if (dirLight.ambient != 0) obj.props.push(new NameValuePair("ambient", dirLight.ambient));
		if (dirLight.castsShadows) obj.props.push(new NameValuePair("castsShadows", dirLight.castsShadows));
		if (dirLight.color) obj.props.push(new NameValuePair("color", "0x" + Std.string(dirLight.color)));
		if (dirLight.ambientColor) obj.props.push(new NameValuePair("ambientColor", "0x" + Std.string(dirLight.ambientColor)));
		if (dirLight.diffuse) obj.props.push(new NameValuePair("diffuse", dirLight.diffuse));
		if (dirLight.specular) obj.props.push(new NameValuePair("specular", dirLight.specular));
	...
	//===========================================================================    
	//===== PARSE LIGHTS ======================================================== 	
	private static function getCamera3DData(camera : Camera3D, obj : SerializedObject) : SerializedObject
	{
	...
	public static function getMeshData(mesh : Mesh, obj : SerializedObject) : SerializedObject
	{
	...
	public static function getObject3dData(object3d : Object3D, obj : SerializedObject = null) : SerializedObject
	{
		if (obj == null) obj = new SerializedObject();
		if (object3d.name != null && object3d.name != "" && object3d.name != "null") {
			var typeName : String = getClassName(object3d);
			if (object3d.name != typeName.toLowerCase()) obj.props.push(new NameValuePair("name", object3d.name));
		}  
		// Pivot 
		var px : Float = mathRound(object3d.pivotPoint.x, 2);
		var py : Float = mathRound(object3d.pivotPoint.y, 2);
		var pz : Float = mathRound(object3d.pivotPoint.z, 2);
		if (px != 0 || py != 0 || pz != 0) {
			obj.props.push(new NameValuePair("pivotPoint", new Vector3D(px, py, pz)));
		}  
		// Position  
		var x : Float = mathRound(object3d.position.x, 2);
		var y : Float = mathRound(object3d.position.y, 2);
		var z : Float = mathRound(object3d.position.z, 2);
		if (x != 0 || y != 0 || z != 0) {
			obj.props.push(new NameValuePair("position", new Vector3D(x, y, z)));
		}  
		// Rotations  
		var rotationX : Float = mathRound(object3d.rotationX, 2);
		var rotationY : Float = mathRound(object3d.rotationY, 2);
		var rotationZ : Float = mathRound(object3d.rotationZ, 2);
		if (Math.round(object3d.rotationX) != 0) obj.props.push(new NameValuePair("rotationX", rotationX));
		if (Math.round(object3d.rotationY) != 0) obj.props.push(new NameValuePair("rotationY", rotationY));
		if (Math.round(object3d.rotationZ) != 0) obj.props.push(new NameValuePair("rotationZ", rotationZ))  
		// Scaling 
		var scaleX : Float = mathRound(object3d.scaleX, 2);
		var scaleY : Float = mathRound(object3d.scaleY, 2);
		var scaleZ : Float = mathRound(object3d.scaleZ, 2);
		if (scaleX != 1) obj.props.push(new NameValuePair("scaleX", scaleX));
		if (scaleY != 1) obj.props.push(new NameValuePair("scaleY", scaleY));
		if (scaleZ != 1) obj.props.push(new NameValuePair("scaleZ", scaleZ));
		return obj;
	}  
	...
	private static function createInstanceName(type : Class<Dynamic>, id : Int) : String
	{
		var name : String = getClassName(type);
		return name.toLowerCase() + id;
	}
	
	private static function mathRound(value : Float, decimalPlaces : Int) : Float
	{
		var factor : Int = Math.pow(10, decimalPlaces);
		return as3hx.Compat.parseInt((value) * factor) / factor;
	}  
	
	// return the Class type for the object  
	private static function getType(object : Dynamic) : Class<Dynamic>
	{
		var classPath : String = getClassPath(object);
		return cast((Type.resolveClass(classPath)), Class);
	}  
	...
	private static function getClassPath(object : Dynamic) : String
	{
		return flash.utils.getQualifiedClassName(object).replace("::", ".");
	}
	
	private static function convertDictionaryToArray(dict : Dictionary) : Array<Dynamic>
	{
		var array : Array<Dynamic> = [];
		for (obj/* AS3HX WARNING could not determine type for var: obj exp: EIdent(dict) type: Dictionary */ in dict) {
			array.push(obj);
		}
