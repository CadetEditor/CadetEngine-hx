...
import cadet3d.serialize.ColorMaterial;
import cadet3d.serialize.CubeGeometry;
import cadet3d.serialize.ISerializer;
import cadet3d.serialize.NameValuePair;
import cadet3d.serialize.Object3D;
...
class AS3Serializer implements ISerializer
{
	...
	public function export(object3d : Object3D) : String
	{
		...
		var geometries : Array<Dynamic> = SceneParser.geometries;
		...
		var numSegmentSets : Int = SceneParser.numSegmentSets;
		var str : String = "";
	...
	{  
		...
		for (i in 0...obj.constructorArgs.length) {
			var nvp : NameValuePair = obj.constructorArgs[i];
			var nvps : Array<NameValuePair> = convertNVP(nvp);
			for (j in 0...nvps.length) {
				nvp = nvps[j]; str += nvp.value;
	...
	private function convertNVP(nvp : NameValuePair) : Array<NameValuePair>
	{
		var name : String = nvp.name;
		var value : Dynamic = nvp.value;
		var nvps : Array<NameValuePair> = new Array<NameValuePair>();
		if (Std.is(nvp.value, Array/*Vector.<T> call?*/)) {
			if (nvp.name == "segments") {
				var segments : Array<SerializedObject> = nvp.value;
		...
		if (Std.is(nvp.value, SerializedObject)) {
			if (nvp.name == "geometry") {
				value = "geometries[" + cast((nvp.value), SerializedObject).id + "]";
			} else if (nvp.name == "material") {
				value = "materials[" + cast((nvp.value), SerializedObject).id + "]";
			}
		} else if (Std.is(nvp.value, Vector3D)) {
			value = "new " + Std.string(cast((nvp.value), Vector3D));
		}
		nvp.name = name;
		nvp.value = value;
		nvps.push(nvp);
	...
	private function writeStartOfFile() : String
	{
		...
		if (addHoverCamera) str += newLine("	import away3d.controllers.HoverController;");
		...
		if (addHoverCamera) str += newLine("			cameraController = new HoverController(camera);");
		str += newLine();
	...
	private function writeEndOfFile() : String
	{
		...
		str += newLine("		{"); str += newLine("			view.width = stage.stageWidth;");
