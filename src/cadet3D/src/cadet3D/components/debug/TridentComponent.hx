...
class TridentComponent extends ObjectContainer3DComponent
{
	...
	private var _showLetters : Bool;
	private static inline var DISPLAY : String = "display";
	
	public function new(length : Int = 500, showLetters : Bool = true)
	{
		super();
	...
	private function set_showLetters(value : Bool) : Bool
	{
	...
	private function get_showLetters() : Bool
	{
		return _showLetters;
	}
	...
	private function buildTrident(length : Float, showLetters : Bool) : Void
	{
		...
		var colors : Array<Int> = [0xFF0000, 0x00FF00, 0x0000FF];
		...
		merge.applyToMeshes(_mesh, [arrowX, arrowY, arrowZ, origin]);
		if (showLetters) { 
			...
			//x  
			vectors[3] = [new Vector3D(length + scl2, scl1, 0), 
						new Vector3D(length + scl3, -scl1, 0), 
						new Vector3D(length + scl3, scl1, 0), 
						new Vector3D(length + scl2, -scl1, 0)];  
			//y  
			vectors[4] = [new Vector3D( -scaleW * 1.2, length + scl4, 0), 
			...
			//z  
			vectors[5] = [new Vector3D(0, scl1, length + scl2), 
