...
class HoverCamProcess extends Component implements ISteppableComponent
{
	...
	private function set_currentPanAngle(value : Float) : Float
	{
		_controller.currentPanAngle = value;
		return value;
	}
	
	private function get_currentPanAngle() : Float
	{
	...
	private function set_currentTiltAngle(value : Float) : Float
	{
		_controller.currentTiltAngle = value;
		return value;
	}
	
	private function get_currentTiltAngle() : Float
	{
	...
	private function set_lookAtComponent(target : ObjectContainer3DComponent) : ObjectContainer3DComponent
	{
		_lookAtComponent = target;
		if (_lookAtComponent != null) {
			_controller.lookAtObject = cast((target.object3D), ObjectContainer3D);
		} else {
	...
	private function get_lookAtComponent() : ObjectContainer3DComponent
	{
	...
	private function set_targetComponent(target : ObjectContainer3DComponent) : ObjectContainer3DComponent
	{
		_targetComponent = target;
		if (_targetComponent != null) {
			_controller.targetObject = cast((target.object3D), Entity);
		} else {
	...
	private function get_targetComponent() : ObjectContainer3DComponent
	{
	...
	public function step(dt : Float) : Void
	{
		...
		if (_targetComponent != null && _lookAtComponent != null) {
			_controller.update();
		}
	}
}