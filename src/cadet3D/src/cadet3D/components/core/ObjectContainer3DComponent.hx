...
@:meta(Event(name="object3DAdded",type="cadetAway3D4.events.Object3DComponentEvent"))
...
class ObjectContainer3DComponent extends ComponentContainer
{
	...
	override private function childAdded(child : IComponent, index : Int) : Void
	{
		...
		var object3DComponent : ObjectContainer3DComponent = try cast(child, ObjectContainer3DComponent) catch (e:Dynamic) null;
		if (object3DComponent == null) return;
		_object3D.addChild(object3DComponent.object3D);
	}
	
	override private function childRemoved(child : IComponent) : Void
	{
	...
	private function set_serializedTransform(value : String) : String
	{
		var split : Array<Dynamic> = value.split(",");
		var v : Array<Float> = new Array<Float>();
		
		for (i in 0...split.length) {
			v.push(Std.parseFloat(split[i]));
		}
	...
	private function get_serializedTransform() : String
	{
		var m : Matrix3D = transform;
		var output : String = "";
		
		for (i in 0...m.rawData.length) {
	...
	private function set_transform(value : Matrix3D) : Matrix3D
	{
