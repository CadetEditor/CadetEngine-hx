...
class DebugDrawProcess extends Component implements IDebugDrawProcess
{
	...
	private static inline var RENDERER : String = "renderer";
	private static inline var PHYSICS : String = "physics";
	private var _renderer : Renderer2D;
	private var _physicsProcess : PhysicsProcess;
	private var _dbgDraw : B2DebugDraw;
	private var _drawScale : Float = 100;
	private var _fillAlpha : Float = 0.3;
	private var _lineThickness : Float = 1;
	private var _trackCamera : Bool = false;
	private var _sprite : Sprite;
	
	public function new(name : String = "DebugDrawProcess")
    {
		super(name); 
	...
	private function set_DrawScale(value : Float) : Float
	{
		_drawScale = value;
		invalidate(DISPLAY);
        return value;
	...
	private function get_DrawScale() : Float
	{
		return _drawScale;
    }
	...
	private function set_TrackCamera(value : Bool) : Bool
	{
		_trackCamera = value;
        return value;
    }
	
	private function get_TrackCamera() : Bool
	{
		return _trackCamera;
    }
	...
	private function set_Renderer(value : IRenderer2D) : IRenderer2D
	{
		...
		if (_renderer != null && !_renderer.initialised) {
	...
	{
		_sprite = cast((_renderer.getNativeParent()), Sprite);
		if (_renderer.initialised && _sprite != null) {
			_dbgDraw.SetSprite(_sprite);
        }
    }
	
	private function validatePhysics() : Bool
	{
	...
	private function get_Sprite() : Dynamic
	{
		return _sprite;
    }
