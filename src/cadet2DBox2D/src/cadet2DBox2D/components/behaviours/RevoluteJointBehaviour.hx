...
class RevoluteJointBehaviour extends Component implements IJoint
{
	...
	private static var DEG_TO_RAD : Float = Math.PI / 180;
	...
	private var _maxMotorTorque : Float = 1;
	...
	@:meta(Serializable())
	@:meta(Inspectable())
	private function set_LowerAngle(value : Float) : Float
	{
		_lowerAngle = value;
		if (joint != null) joint.SetLimits(_lowerAngle * DEG_TO_RAD, _upperAngle * DEG_TO_RAD);
        else invalidate(JOINT);
        return value;
    }
	
	private function get_LowerAngle() : Float
	{
		return _lowerAngle;
    }
	...
	@:meta(Serializable())
	@:meta(Inspectable())
	private function set_UpperAngle(value : Float) : Float
	{
		_upperAngle = value;
		if (joint != null) joint.SetLimits(_lowerAngle * DEG_TO_RAD, _upperAngle * DEG_TO_RAD);
        else invalidate(JOINT);
	...
	private function get_UpperAngle() : Float
	{
		return _upperAngle;
    }
	...
	@:meta(Serializable())
	@:meta(Inspectable())
	private function set_MaxMotorTorque(value : Float) : Float
	{
		_maxMotorTorque = value;
		if (joint != null) joint.SetMaxMotorTorque(_maxMotorTorque);
        else invalidate(JOINT);
        return value;
    }
	
	private function get_MaxMotorTorque() : Float
	{
	...
	private function validateJoint() : Void
	{
		...
		if (_physicsProcess == null) return;
		if (_pin == null || !_pin.localPos) return;
		physicsBehaviourA.validateNow();
		physicsBehaviourB.validateNow();
		var jointDef : B2RevoluteJointDef = new B2RevoluteJointDef();
		jointDef.collideConnected = collideConnected; jointDef.enableLimit = _enableLimit; 
