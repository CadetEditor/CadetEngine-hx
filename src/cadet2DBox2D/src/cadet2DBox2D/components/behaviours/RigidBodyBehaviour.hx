...
@:meta(Event(type="cadet2DBox2D.events.RigidBodyBehaviourEvent",name="destroyRigidBody"))
class RigidBodyBehaviour extends Component implements ISteppableComponent
{
	...
	private static inline var SHAPES : String = "shapes";
	private static var helperMatrix : Matrix = new Matrix();
	private var _density : Float = 1;
	private var _friction : Float = 0.8;
	private var _restitution : Float = 0.5;
	private var _fixed : Bool = false;
	private var _angularDamping : Float = 0.1;
	private var _linearDamping : Float = 0.1;
	private var _categoryBits : Int = 1;
	private var _maskBits : Int = Int.MAX_VALUE;
	private var _geometry : IGeometry;
	private var _transform : Transform2D;
	private var _physicsProcess : PhysicsProcess;
	private var body : B2Body;
	private var storedTranslateX : Float;
	private var storedTranslateY : Float;
	private var storedScaleX : Float;
	private var storedScaleY : Float;
	private var matrix : Matrix;
	private var tempVecA : B2Vec2;
	private var tempVecB : B2Vec2;
	
	public function new(fixed : Bool = false, density : Float = 1, friction : Float = 0.8, restitution : Float = 0.5)
    {
        super();
	...
	public function applyImpulse(impulseX : Float, impulseY : Float) : Void
	{
		var body : B2Body = getBody();
		if (body == null) return;
	...
	public function applyImpulseAt(impulseX : Float, impulseY : Float, worldPosX : Float, worldPosY : Float) : Void
	{
		var body : B2Body = getBody();
		if (body == null) return;
		tempVecA.Set(impulseX, impulseY);
	...
	private function buildShape(geometry : IGeometry, body : B2Body) : Void
	{
		geometry.validateNow();
		var globalMatrix : Matrix = _transform.globalMatrix;
		var globalScaleX : Float = Math.sqrt(globalMatrix.a * globalMatrix.a + globalMatrix.b * globalMatrix.b);
		var globalScaleY : Float = Math.sqrt(globalMatrix.c * globalMatrix.c + globalMatrix.d * globalMatrix.d);
		
		if (Std.is(geometry, CircleGeometry)) {  
			// Take the scale transform of the circle into account  
			var scale : Float = globalScaleX > (globalScaleY != 0) ? globalScaleX : globalScaleY;
			var circle : CircleGeometry = cast((geometry), CircleGeometry);
			...
			circleShapeDef.density = (fixed) ? 0 : density; circleShapeDef.localPosition = new B2Vec2(circle.x * _physicsProcess.scaleFactor, circle.y * _physicsProcess.scaleFactor);
			body.CreateShape(circleShapeDef);
        } else if (Std.is(geometry, PolygonGeometry)) {
			var polygon : PolygonGeometry = cast((geometry), PolygonGeometry);
			var vertices : Array<Dynamic> = VertexUtil.copy(polygon.vertices);
			if (globalScaleX != 1 || globalScaleY != 1) {
				helperMatrix.identity();helperMatrix.scale(globalScaleX, globalScaleY);  
			...
			var allVertices : Array<Dynamic> = [vertices];  
			...
			for (childData/* AS3HX WARNING could not determine type for var: childData exp: EField(EIdent(compoundGeometry),children) type: null */ in compoundGeometry.children) {
	...
	@:meta(Serializable())
	private function set_CategoryBits(value : Int) : Int
	{
		_categoryBits = value;
		if (body != null) {
			invalidate(SHAPES);
	...
	private function get_CategoryBits() : Int
	{
		return _categoryBits;
    }
	
	@:meta(Serializable())
	private function set_MaskBits(value : Int) : Int
	{
		_maskBits = value;
		if (body != null) {
			invalidate(SHAPES);
	...
	private function get_MaskBits() : Int
	{
		return _maskBits;
    }
}