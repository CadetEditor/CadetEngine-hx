...
import nme.events.Event;import nme.geom.Point;import box2d.common.math.B2Vec2;import box2d.dynamics.joints.B2DistanceJointDef;import box2d.dynamics.joints.B2Joint;import cadet.core.Component;import cadet.events.ComponentEvent;import cadet.events.ValidationEvent;import cadet.util.ComponentUtil;import cadet2d.components.connections.Connection;import cadet2dbox2d.components.processes.PhysicsProcess;class DistanceJointBehaviour extends Component implements IJoint
{
    public var connection(get, set) : Connection;
    public var physicsProcess(get, set) : PhysicsProcess;
  // Invalidation types  private static inline var JOINT : String = "joint";private static inline var BEHAVIOURS : String = "behaviours";private var _connection : Connection;private var _physicsProcess : PhysicsProcess;private var joint : B2Joint;private var physicsBehaviourA : RigidBodyBehaviour;private var physicsBehaviourB : RigidBodyBehaviour;@:meta(Serializable())
public var length : Float = -1;@:meta(Serializable())
@:meta(Inspectable())
public var damping : Float = 0.5;@:meta(Serializable())
@:meta(Inspectable())
public var collideConnected : Bool = true;public function new()
    {
        super();name = "DistanceJointBehaviour";
