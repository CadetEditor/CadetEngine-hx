...
class SpringBehaviour extends Component implements ISteppableComponent
{
	...
	private var _rigidBodyBehaviourA : RigidBodyBehaviour;
	private var _rigidBodyBehaviourB : RigidBodyBehaviour;
	
	@:meta(Serializable())
	@:meta(Inspectable())
	public var length : Float = 100;  //-1;  
	
	@:meta(Serializable())
	@:meta(Inspectable())
	public var stiffness : Float = 0.5;
	private var len : Float = -1;  
	...
	{
		if (physicsProcess == null) return;
		var friction : Float = 0.2;
		var k : Float = stiffness * 100;
		...
		var dx : Float = diff.Normalize();
		var vRel : Float = vDiff.x * diff.x + vDiff.y * diff.y;
		var forceMag : Float = -k * (dx - len) - friction * vRel;
