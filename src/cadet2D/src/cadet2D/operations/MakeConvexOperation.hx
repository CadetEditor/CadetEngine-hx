...
class MakeConvexOperation implements IUndoableOperation
{
	...
	public function new(polygon : PolygonGeometry)
    {
	...
	public function execute() : Void
	{
		...
		if (VertexUtil.isConcave(polygon.vertices) == false) {
			_result = polygon;return;
        }
		
		var compoundGeometry : CompoundGeometry = new CompoundGeometry();
		compoundGeometry.name = polygon.name;
		var decomposedVertices : Array<Dynamic> = VertexUtil.makeConvex(VertexUtil.copy(polygon.vertices));
		
		for (convexVertices in decomposedVertices) {
	...
	public function getResult() : IGeometry
	{
		return _result;
    }
