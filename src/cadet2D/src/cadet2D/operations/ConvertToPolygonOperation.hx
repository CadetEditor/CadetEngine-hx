...
class ConvertToPolygonOperation implements IUndoableOperation
{
	...
	private var index : Int;public function new(polygon : PolygonGeometry)
    {
		this.polygon = polygon; 
		var type : Class<Dynamic> = IntrospectionUtil.getType(polygon);
		if (type == PolygonGeometry) {
			result = polygon;
        } else {
			result = new PolygonGeometry();
			result.name = polygon.name;
			result.exportTemplateID = polygon.exportTemplateID; result.templateID = polygon.templateID;
			var clonedVertices : Array<Dynamic> = VertexUtil.copy(polygon.vertices);
