...
class CenterOriginOperation extends UndoableCompoundOperation
{
	private var geometry : IGeometry;
	private var transform : Transform2D;
	private var useCentroid : Bool;
	
	public function new(geometry : IGeometry, transform : Transform2D, useCentroid : Bool = false)
    {
        super();
		...
		if (Std.is(geometry, CircleGeometry)) {
			centerCircle(cast((geometry), CircleGeometry));
        } else if (Std.is(geometry, PolygonGeometry)) {
			centerPolygon(cast((geometry), PolygonGeometry));
        } else if (Std.is(geometry, BezierCurve)) {
			centerBezierCurve(cast((geometry), BezierCurve));
        }
    }
