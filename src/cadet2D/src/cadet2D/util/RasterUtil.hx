...
class RasterUtil
{
	private static var labelTable : Array<Dynamic>;
	private static var loops : Array<Dynamic>;
	private static inline var UNLABELED : Int = 0;
	private static var VISITED_WHITE : Int = -1;
	private static var offsetTableX : Dynamic;
	private static var offsetTableY : Dynamic;
	private static var nextPosTable : Dynamic;
	
	public static function bitmapToVertices(bmpData : BitmapData) : Array<Dynamic>
	{
		if (offsetTableX == null) {
			offsetTableX = { };
			offsetTableY = { };
			nextPosTable = { };
			offsetTableX[0] = 1;
		...
		for (y in 1...h) {
			for (x in 0...w) {
				var isBlack : Bool = bitmap.getPixel(x, y) == 0;
				if (!isBlack) {
					continue;
                }
				...
				var isPixelAboveBlack : Bool = bitmap.getPixel(x, y - 1) == 0;
				var isContour : Bool = false;  
				// Step 1  
				...
				var isPixelBelowBlack : Bool = bitmap.getPixel(x, y + 1) == 0;
	...
	private static function traceContour(x : Int, y : Int, C : Int, bitmap : BitmapData, contourType : Bool) : Void
	{
		...
		do { 
			...
			while (i-- > -1) {
				...
				var isLocalBlack : Bool = bitmap.getPixel(localX, localY) == 0;
				if (isLocalBlack) {
