...
class VertexUtil
{
	...
	public static function simplify(vertices : Array<Dynamic>, tolerance : Float = 1) : Array<Dynamic>
	{
		var marks : Array<Dynamic> = [];
		marks[0] = marks[vertices.length - 1] = true;
		simplifyRecur(vertices, tolerance, 0, vertices.length - 1, marks);
		var newVertices : Array<Dynamic> = [];
		var m : Int = 0;
	...
	private static function simplifyRecur(vertices : Array<Dynamic>, tolerance : Float, j : Int, k : Int, marks : Array<Dynamic>) : Void
	{
		if (k <= j + 1) return;
		var maxi : Int = j;
		var maxd2 : Float = 0;
	...
	public static function transform(vertices : Array<Dynamic>, matrix : Matrix) : Void
	{
	...
	public static function copy(vertices : Array<Dynamic>) : Array<Dynamic>
	{
		var newVertices : Array<Dynamic> = [];
	...
	public static function getBounds(vertices : Array<Dynamic>, rect : Rectangle = null) : Rectangle
	{
		...
		for (vertex in vertices) {
			minX = vertex.x < (minX != 0) ? vertex.x : minX;
			minY = vertex.y < (minY != 0) ? vertex.y : minY;
			maxX = vertex.x > (maxX != 0) ? vertex.x : maxX;
			maxY = vertex.y > (maxY != 0) ? vertex.y : maxY;
        }
		
		if (rect == null) return new Rectangle(minX, minY, maxX - minX, maxY - minY);
		rect.left = minX;
		rect.right = maxX;
		rect.top = minY;
		rect.bottom = maxY;
	...
	public static function hittest(px : Float, py : Float, vertices : Array<Dynamic>) : Bool
	{
		return windingNumber(px, py, vertices) != 0;
    }
	
	public static function windingNumber(px : Float, py : Float, vertices : Array<Dynamic>) : Int
	{
		var wn : Int = 0;
		var L : Int = vertices.length;
	...
	public static function isConvex(vertices : Array<Dynamic>) : Bool
	{
		return !isConcave(vertices);
    }
	
	public static function isConcave(vertices : Array<Dynamic>, clockwise : Bool = true) : Bool
	{
		...
		for (i in 0...n) {
			var j : Int = (i + 1) % n;
			var k : Int = (i + 2) % n;
			var z : Float = (vertices[j].x - vertices[i].x) * (vertices[k].y - vertices[j].y); 
	...
	public static function getArea(vertices : Array<Dynamic>) : Float
	{
		var n : Int = vertices.length;
		var a : Float = 0;
	...
	public static function computeCentroid(vertices : Array<Dynamic>) : Vertex
	{
		...
		for (i in 0...length) {
			...
			var p3 : Vertex = i + 1 >= (length != 0) ? vertices[0] : vertices[i + 1];
	...
	public static function getClosestVertex(x : Float, y : Float, vertices : Array<Dynamic>) : Vertex
	{
		var closestDistance : Float = Float.MAX_VALUE;
		var closestVertex : Vertex;
		
		for (vertex in vertices) {
			var dx : Float = vertex.x - x;
			var dy : Float = vertex.y - y;
			var d : Float = dx * dx + dy * dy;
	...
	public static function getIntersections(verticesA : Array<Dynamic>, verticesB : Array<Dynamic>) : Array<Dynamic>
	{
		var intersections : Array<Dynamic> = [];
		
		for (i in 0...verticesA.length) {
			var vertexA_1 : Vertex = verticesA[i];
			var vertexA_2 : Vertex = verticesA[i == verticesA.length - (1) ? 0 : i + 1];
			
			for (j in 0...verticesB.length) {
	...
	public static function intersection(a0 : Vertex, a1 : Vertex, b0 : Vertex, b1 : Vertex, limit : Bool = true) : Vertex
	{
		...
		var D : Float = u.x * v.y - u.y * v.x;
		if (Math.abs(D) < EPSILON) return null;
		var sI : Float = (v.x * w.y - v.y * w.x) / D;
		if (limit) {
			if (sI < 0 || sI > 1) return null;
			var tI : Float = (u.x * w.y - u.y * w.x) / D;if (tI < 0 || tI > 1) return null;
        }
	...
	public static function isLeft(v0 : Vertex, v1 : Vertex, px : Float, py : Float) : Bool
	{
		return ((v1.x - v0.x) * (py - v0.y) - (px - v0.x) * (v1.y - v0.y)) < 0;
    }
	
	@:allow(cadet2d.util)
    private static function toVertexList(vertices : Array<Dynamic>) : VertexList
	{
	...
	public static function makeConvex(vertices : Array<Dynamic>) : Array<Dynamic>
	{
		var returnShapes : Array<Dynamic> = [];
		var bounds : Rectangle = getBounds(vertices);
		var diameter : Float = Math.sqrt(bounds.width * bounds.width + bounds.height * bounds.height);
		var openList : Array<Dynamic> = [toVertexList(vertices)];
		...
		while (openList.length > 0) {
			...
			var currentShape : Array<Dynamic> = currentShapeStart.toArray();
			...
			do {
				...
				// Found notch 
				var dx : Float = (v.next.x - v.x);
				var dy : Float = (v.next.y - v.y);
				...
				var isectInfo : Dynamic = getClosestLineToShapeIntersection(ray1, ray2, currentShapeStart, 0);
				...
				if (isectInfo.d < 1) {
					...
					cutoutIsect.next = v.next;
					vertexBeforeIsect.next = cutoutIsect; openList.push(cutoutIsect);
	...
	private static function cloneList(start : VertexList) : VertexList
	{
	...
	private static function getPrevVertex(vertex : VertexList) : VertexList
	{
	...
	private static function getClosestLineToShapeIntersection(v1 : Vertex, v2 : Vertex, shapeStart : VertexList, minDistance : Float) : Dynamic
	{
		var v : VertexList = shapeStart;
		var closestDistance : Float = Float.POSITIVE_INFINITY;
		var closestIsect : Dynamic;
		
		do { 
			var isect : Vertex = intersection(v1, v2, v, v.next);
			if (isect != null) {
				var dx : Float = v1.x - isect.x;
				var dy : Float = v1.y - isect.y;
				var d : Float = Math.sqrt(dx * dx + dy * dy);
	...
	public static function getPolygonStrip(vertices : Array<Dynamic>, thickness : Float, offset : Float = 0) : Array<Dynamic>
	{
		var shapes : Array<Dynamic> = [];
		var halfThickness : Float = thickness * 0.5;
		var L : Int = vertices.length - 1;
		
		for (i in 0...L) {
			var vertex : Vertex = vertices[i];
			var nextVertex : Vertex = vertices[i + 1];
			var dx : Float = nextVertex.x - vertex.x;
			var dy : Float = nextVertex.y - vertex.y;
			var d : Float = Math.sqrt(dx * dx + dy * dy); 
			dx /= d; dy /= d;
			...
			var shape : Array<Dynamic> = [v0, v1, v2, v3];
			
			if (i > 0) {
				var prevShape : Array<Dynamic> = shapes[i - 1];
