...
class MovieClipSkin extends ImageSkin implements IAnimatable
{
	...
	private var _loopDirty : Bool;
	private var _addedToJuggler : Bool;
	private var _previewAnimation : Bool;
	
	public function new(name : String = "MovieClipSkin")
    {
		super(name);
    }
	...
	{
		var isInvalidLoop : Bool = isInvalid(LOOP);  
	...
	override private function createQuad(textures : Array<Texture>) : Quad
	{
	...
	{
		if (renderer == null || _quad == null) {
			_loopDirty = true;return;
        }
		var success : Bool;
		if (_loop) {
	...
	private function get_Movieclip() : MovieClip
	{
		return cast((_quad), MovieClip);
    }
