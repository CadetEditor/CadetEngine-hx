...
class SpringSkin extends AbstractSkin2D
{
	...
	private var _lineColor : Float;
	private var _lineAlpha : Float;
	private var _numZigZags : Float;
	private var _width : Float;
	private var _connection : Connection;
	private var _renderer : Renderer2D;
	private var _shape : Shape;
	
	public function new(lineThickness : Float = 1, lineColor : Int = 0xFFFFFF, lineAlpha : Float = 0.5, numZigZags : Int = 20, width : Float = 10)
    {
        super();
		name = "SpringSkin";
	...
	override private function validateDisplay() : Bool
	{
		...
		if (_renderer == null || !_renderer.viewport) return false;
		...
		var switcher : Int = 1;
		var nx : Float = pt2.x - pt1.x;
		var ny : Float = pt2.y - pt1.y;
		var m : Float = Math.sqrt(nx * nx + ny * ny);
		var n : Point = new Point((ny / m) * width * 0.5, -(nx / m) * width * 0.5);
		for (i in 0...numZigZags) {
			var ratio : Float = (i + 1) / (numZigZags + 1);
			var ptx : Float = pt1.x + ratio * nx;
			var pty : Float = pt1.y + ratio * ny;
			ptx += n.x * switcher;
			pty += n.y * switcher;
	...
	@:meta(Serializable())
	@:meta(Inspectable())
	private function set_NumZigZags(value : Float) : Float
	{
		_numZigZags = value;
		invalidate(DISPLAY);
        return value;
	...
	private function get_NumZigZags() : Float
	{
		return _numZigZags;
    }  
