...
class TerrainSkin extends AbstractSkin2D
{
	...
	// Invalidation types  
	public static inline var ALL_BUCKETS : String = "allBuckets";
	public static inline var SOME_BUCKETS : String = "someBuckets";  
	// Sibling References  
	private var _terrainGeometry : TerrainGeometry;  
	// Styles    
	//protected var _surfaceBitmap		:BitmapData;  
	private var _surfaceThickness : Int = 20;  
	//protected var _fillBitmap			:BitmapData;  
	private var _surfaceTexture : TextureComponent;
	private var _fillTexture : TextureComponent;  
	private var invalidatedBuckets : Dynamic;
	private var shapes : Dynamic;
	private static var m : Matrix = new Matrix();
	private var _shape : Shape;
	
	public function new(name : String = "TerrainSkin")
    {
		super(name);
	...
	private function set_FillTexture(value : TextureComponent) : TextureComponent
	{
	...
	private function get_FillTexture() : TextureComponent
	{
		return _fillTexture;
    }
	...
	private function set_SurfaceTexture(value : TextureComponent) : TextureComponent
	{
	...
	private function get_SurfaceTexture() : TextureComponent
	{
	...
	*/  
	...
	private function set_SurfaceThickness(value : Float) : Float
	{
	...
	private function get_SurfaceThickness() : Float
	{
		return _surfaceThickness;
    }
	
	private function set_TerrainGeometry(value : TerrainGeometry) : TerrainGeometry
	{
		if (_terrainGeometry != null) {
	...
	private function get_TerrainGeometry() : TerrainGeometry
	{
	...
	{
		if (_terrainGeometry == null) return;
		invalidatedBuckets = { };
		var numBuckets : Int = Math.ceil(_terrainGeometry.numSamples / _terrainGeometry.bucketSize);
		for (i in 0...numBuckets) {
			invalidatedBuckets[i] = true;
        }
		for (shape/* AS3HX WARNING could not determine type for var: shape exp: EIdent(shapes) type: Dynamic */ in shapes) {
	...
	private function validateSomeBuckets() : Void
	{
		...
		for (bucketIndex in Reflect.fields(invalidatedBuckets)) {
			validateBucket(as3hx.Compat.parseInt(bucketIndex));
        }
		var terrainInvalidatedBuckets : Dynamic = _terrainGeometry.invalidatedBuckets;
		for (bucketIndex in Reflect.fields(terrainInvalidatedBuckets)) {
			if (Reflect.field(invalidatedBuckets, bucketIndex)) continue;
			validateBucket(as3hx.Compat.parseInt(bucketIndex));
        }
		invalidatedBuckets = { };
    }
	...
	private function validateBucket(index : Int) : Void
	{
		...
		var samples : Array<Dynamic> = _terrainGeometry.samples;
		var sampleWidth : Float = _terrainGeometry.sampleWidth;
		var firstSampleIndex : Int = index * _terrainGeometry.bucketSize;
		var lastSampleIndex : Int = Math.min(firstSampleIndex + _terrainGeometry.bucketSize, samples.length - 1);
		var length : Int = lastSampleIndex - firstSampleIndex;
		shape.x = as3hx.Compat.parseInt(firstSampleIndex * sampleWidth);
		var graphics : Graphics = shape.graphics;graphics.clear();  
		//if ( _fillBitmap )  
		if (_fillTexture != null) {
			m.identity();
			m.tx = as3hx.Compat.parseInt(-shape.x % _fillTexture.texture.width);  
		...
		for (i in 0...length + 1) {
			var sample : Float = samples[firstSampleIndex + i];
			graphics.lineTo(as3hx.Compat.parseInt(i * sampleWidth), -sample);
			vertices[i] = new Vertex(as3hx.Compat.parseInt(i * sampleWidth), -sample);
        }
		graphics.lineTo(as3hx.Compat.parseInt((i - 1) * sampleWidth), 0);
		graphics.endFill();  
		//if ( !_surfaceBitmap ) return;  
		if (_surfaceTexture == null)  return;
		var strip : Array<Dynamic> = VertexUtil.getPolygonStrip(vertices, _surfaceThickness, 0);
		for (i in 0...length) { 
			sample = samples[firstSampleIndex + i];
			var stripShape : Array<Dynamic> = strip[i];
			var dx : Float = stripShape[1].x - stripShape[0].x;
			var dy : Float = stripShape[1].y - stripShape[0].y;
			var d : Float = Math.sqrt(dx * dx + dy * dy);
			dx /= d;
			dy /= d;
			var angle : Float = Math.PI * 0.5 - Math.atan2(dx, dy);m.identity();  
