...
class TrackCamera2DProcess extends Component implements ISteppableComponent
{
	...
	private var snapTo : Bool = false;
	private var matrix : Matrix;
	private var cameraRect : Rectangle;
	private var oldCameraRect : Rectangle;
	private var oldObjectX : Float;
	private var oldObjectY : Float;
	...
	private function set_Target(value : IComponentContainer) : IComponentContainer
	{
		_target = value;
		snapTo = true;
        return value;
	...
	private function get_Target() : IComponentContainer
	{
		return _target;
    }
	
	private function set_Renderer(value : IRenderer2D) : IRenderer2D
	{
		_renderer = value;
		snapTo = true;
        return value;
	...
	private function get_Renderer() : IRenderer2D
	{
		return _renderer;
    }
	
	private function set_WorldBounds(value : WorldBounds2D) : WorldBounds2D
	{
		_worldBounds = value;
        return value;
    }
	
	private function get_WorldBounds() : WorldBounds2D
	{
		return _worldBounds;
    }
	
	private function set_DebugDrawProcess(value : IDebugDrawProcess) : IDebugDrawProcess
	{
		_debugDrawProcess = value;
        return value;
    }
	
	private function get_DebugDrawProcess() : IDebugDrawProcess
	{
		return _debugDrawProcess;
    }
	
	public function step(dt : Float) : Void
	{
		if (_renderer == null || !_renderer.initialised) return;
		...
		var bounds : Rectangle = cast((skin), AbstractSkin2D).displayObject.getBounds(cast((cast((_renderer), Renderer2D).worldContainer), DisplayObject));
		if (bounds.width == 0) return;
		if (bounds.height == 0) return;
		var viewportWidth : Float = _renderer.viewportWidth;
		var viewportHeight : Float = _renderer.viewportHeight;
		var halfViewportWidth : Float = viewportWidth * 0.5;
		var halfViewportHeight : Float = viewportHeight * 0.5;
		var objectX : Float = bounds.left + (bounds.width * 0.5);
		var objectY : Float = bounds.top + (bounds.height * 0.5);
		if (snapTo) {
		...
		var objectVX : Float = objectX - oldObjectX;
		var objectVY : Float = objectY - oldObjectY;
		var objectSpeed : Float = Math.sqrt(objectVX * objectVX + objectVY * objectVY);
		var speedRatio : Float = (objectSpeed - minSpeed) / (maxSpeed - minSpeed);
		speedRatio = speedRatio < (0) ? 0 : speedRatio > (1) ? 1 : speedRatio;speedRatio = Easing.easeOutCubic(speedRatio, 0, 1, 1);
		var desiredZoom : Float = maxZoom + speedRatio * (minZoom - maxZoom);
		var newCameraRect : Rectangle = new Rectangle();
		newCameraRect.left = objectX - halfViewportWidth / desiredZoom; newCameraRect.right = objectX + halfViewportWidth / desiredZoom;
		newCameraRect.top = objectY - halfViewportHeight / desiredZoom;newCameraRect.bottom = objectY + halfViewportHeight / desiredZoom;  
		...
		var rectZoom : Float = Math.max(viewportWidth / cameraRect.width, viewportHeight / cameraRect.height);
		var rectX : Float = cameraRect.x + cameraRect.width * 0.5;
		var rectY : Float = cameraRect.y + cameraRect.height * 0.5;  
		...
		cast((_renderer), Renderer2D).worldContainer.transformationMatrix = matrix;
		_renderer.invalidate("container");
		if (_debugDrawProcess != null && _debugDrawProcess.trackCamera) {
			if (Std.is(_debugDrawProcess.sprite, flash.display.Sprite)) {
				var flashSprite : flash.display.Sprite = flash.display.Sprite(_debugDrawProcess.sprite);
				flashSprite.transform.matrix = matrix;
            }
        }
		oldCameraRect = cameraRect.clone();
		oldObjectX = objectX;oldObjectY = objectY;
    }
}