...
class CollisionDetectionProcess extends Component implements ISteppableComponent
{
	private var boundingSpheres : Array<BoundingSphere>;
	
	public function new()
    {
        super();
		boundingSpheres = new Array<BoundingSphere>();
    }
	
	override private function addedToScene() : Void
	{
		var collisionShapes : Array<IComponent> = ComponentUtil.getChildrenOfType(scene, ICollisionShape, true);
		for (collisionShape in collisionShapes) {
			if (Std.is(collisionShape, BoundingSphere)) {
				boundingSpheres.push(cast((collisionShape), BoundingSphere));
            }
        }
	...
	{
		if (Std.is(event.component, ICollisionShape) == false) return;
		if (Std.is(event.component, BoundingSphere)) {
			boundingSpheres.push(cast((event.component), BoundingSphere));
        }
    }
	...
	private function onComponentRemovedFromScene(event : ComponentEvent) : Void
	{
		if (Std.is(event.component, ICollisionShape) == false) return;
		if (Std.is(event.component, BoundingSphere)) {
			boundingSpheres.splice(Lambda.indexOf(boundingSpheres, cast((event.component), BoundingSphere)), 1);
        }
    }
	
	public function step(dt : Float) : Void
	{
		var collisions : Array<Dynamic> = new Array<Dynamic>();
		var L : Int = boundingSpheres.length;
		for (i in 0...L) {
			var boundingSphereA : BoundingSphere = boundingSpheres[i];
			if (boundingSphereA.transform == null) {
				continue;
            }
			for (j in i + 1...L) {
				var boundingSphereB : BoundingSphere = boundingSpheres[j];
				if (boundingSphereB.transform == null) {
					continue;
                }
				var mA : Matrix = cast((boundingSphereA.transform), Transform2D).globalMatrix;
				var mB : Matrix = cast((boundingSphereB.transform), Transform2D).globalMatrix;
				var dx : Float = mA.tx - mB.tx;
				var dy : Float = mA.ty - mB.ty;
				var sqrScaleXA : Float = mA.a * mA.a + mA.b * mA.b;
				var sqrScaleYA : Float = mA.c * mA.c + mA.d * mA.d;
				var scaleA : Float = sqrScaleXA > (sqrScaleYA != 0) ? Math.sqrt(sqrScaleXA) : Math.sqrt(sqrScaleYA);
				var sqrScaleXB : Float = mB.a * mB.a + mB.b * mB.b;
				var sqrScaleYB : Float = mB.c * mB.c + mB.d * mB.d;
				var scaleB : Float = sqrScaleXB > (sqrScaleYB != 0) ? Math.sqrt(sqrScaleXB) : Math.sqrt(sqrScaleYB);
				var d : Float = dx * dx + dy * dy;
				var r : Float = scaleA * boundingSphereA.radius + scaleB * boundingSphereB.radius;
