...
@:meta(Cadet(inheritFromTemplate="false"))
class Transform2D extends Component implements ITransform2D
{
	...
	private var _parentTransform : Transform2D = null;
	public var dispatchEvents : Bool = false;  
	// Added as a speed optimisation  
	private static inline var TRANSFORM : String = "transform";
	public static inline var PROPERTY_CHANGE_X : String = "propertyChange_x";
	public static inline var PROPERTY_CHANGE_Y : String = "propertyChange_y";
	public static inline var PROPERTY_CHANGE_SCALEX : String = "propertyChange_scaleX";
	public static inline var PROPERTY_CHANGE_SCALEY : String = "propertyChange_scaleY";
	public static inline var PROPERTY_CHANGE_ROTATION : String = "propertyChange_rotation";
	...
	private function set_ParentTransform(value : ITransform2D) : ITransform2D
	{
		...
		setupParentTransform(cast((value), Transform2D));
        return value;
    }
	...
	{
		if (parentTransform != null) throw new Error("parentTransform already set, call cleanUpParentTransform() first");
		if (transform != null) {
			_parentTransform = transform;
			var container : DisplayObjectContainer = cast((_parentTransform._displayObject), DisplayObjectContainer);
			container.addChild(_displayObject);
			invalidate(TRANSFORM);_parentTransform.addEventListener(ValidationEvent.INVALIDATE, onParentTransformInvalidated);
        }
    }
	...
	private function onParentTransformInvalidated(event : ValidationEvent) : Void
	{
		invalidate(TRANSFORM);
    }
}