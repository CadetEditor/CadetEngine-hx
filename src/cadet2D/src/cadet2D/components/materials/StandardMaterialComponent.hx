...
class StandardMaterialComponent extends ComponentContainer implements IMaterialComponent
{
	...
	private inline var VALUES : String = "values";
	private inline var TEXTURES : String = "textures";
	...
	private var _textureComps : Array<TextureComponent>;
	
	public function new(name : String = "StandardMaterialComponent")
    {
		super(name);
		_material = new StandardMaterial();
		texturesContainer = this;
		_textureComps = new Array<TextureComponent>();
    }
	
	@:meta(Serializable())
	@:meta(Inspectable(editor="ComponentList",scope="scene",priority="50"))
	private function set_VertexShader(value : IShaderComponent) : IShaderComponent
	{
		_vertexShader = value;
		invalidate(VALUES);
        return value;
	...
	private function get_VertexShader() : IShaderComponent
	{
	...
	private function set_FragmentShader(value : IShaderComponent) : IShaderComponent
	{
	...
	private function get_FragmentShader() : IShaderComponent
	{
	...
	private function set_TexturesContainer(value : IComponentContainer) : IComponentContainer
	{
		if (_texturesContainer != null) {
	...
	private function get_TexturesContainer() : IComponentContainer
	{
	...
	{
		_material.vertexShader = (_vertexShader != null) ? _vertexShader.shader : null;
		_material.fragmentShader = (_fragmentShader != null) ? _fragmentShader.shader : null;
    }
	
	private function validateTextures() : Void
	{
		for (i in 0..._textureComps.length) {
		...
		_textureComps = new Array<TextureComponent>();
		if (_texturesContainer != null) {
			for (_texturesContainer.children.length) {
				var childComp : IComponent = _texturesContainer.children[i];
				if (Std.is(childComp, TextureComponent)) {
					textureComp = cast((childComp), TextureComponent);
