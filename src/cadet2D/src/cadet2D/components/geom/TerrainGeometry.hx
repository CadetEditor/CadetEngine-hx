...
class TerrainGeometry extends CompoundGeometry
{
	...
	public static inline var SAMPLES : String = "samples";
	...
	private var _samples : Array<Dynamic>;
	private var _buckets : Array<Dynamic>;
	private var _invalidatedBuckets : Dynamic;
	...
	private function get_Buckets() : Array<Dynamic>
	{
		return _buckets;
    }
	
	private function get_InvalidatedBuckets() : Dynamic
	{
		return _invalidatedBuckets;
    }
	
	private function set_Samples(value : Array<Dynamic>) : Array<Dynamic>
	{
		_samples = value;
	...
	private function get_Samples() : Array<Dynamic>
	{
		return _samples;
    }
	
	@:meta(Serializable(alias="samples"))
	private function set_SerializedSamples(value : String) : String
	{
		var split : Array<Dynamic> = value.split(","); s
		amples = [];
		var L : Int = split.length;
		for (i in 0...L) {
			_samples[i] = as3hx.Compat.parseInt(split[i]);
        }
        return value;
    }
	
	private function get_SerializedSamples() : String
	{
		return Std.string(_samples);
    }
	
	@:meta(Serializable())
	@:meta(Inspectable(editor="NumericStepper",min="1",max="100",stepSize="1"))
	private function set_SampleWidth(value : Int) : Int
	{
		_sampleWidth = value;
		invalidate(ALL_BUCKETS);
        return value;
	...
	private function get_SampleWidth() : Int
	{
		return _sampleWidth;
    }
	...
	private function set_BucketSize(value : Int) : Int
	{
		_bucketSize = value;
		invalidate(ALL_BUCKETS);
        return value;
	...
	private function get_BucketSize() : Int
	{
		return _bucketSize;
    }
	
	public function setHeight(index : Int, value : Float) : Void
	{
	...
	private function validateAllBuckets() : Void
	{
		for (bucket in _buckets) {
			for (polygon/* AS3HX WARNING could not determine type for var: polygon exp: EIdent(bucket) type: Dynamic */ in bucket) {
		...
		_buckets = [];
		var numBuckets : Int = Math.ceil((_numSamples / _bucketSize));
	...
	{
		for (bucketIndex in Reflect.fields(_invalidatedBuckets)) {
	...
	{
		var bucket : Array<Dynamic> = _buckets[bucketIndex];
		...
		var firstSampleIndex : Int = bucketIndex * _bucketSize;
		var lastSampleIndex : Int = Math.min(firstSampleIndex + _bucketSize, _samples.length - 1);
		var lowestPoint : Float = Float.POSITIVE_INFINITY;
		for (i in firstSampleIndex...lastSampleIndex + 1) {
			var sample : Float = _samples[i];
			if (sample < lowestPoint) {
				lowestPoint = sample;
            }
        }
		var left : Float = firstSampleIndex * _sampleWidth;
		var right : Float = lastSampleIndex * _sampleWidth;
		if (lowestPoint != 0) {
		...
		for (lastSampleIndex + 1) {
			...
			if (vertex.y == -lowestPoint) {
				...
				var nextSample : Float = _samples[i + 1];
	...
	private function finishShape(polygon : PolygonGeometry, y : Float) : Void
	{
	...
	// during validation.  
	@:meta(Serializable(inherit="false"))
	override private function set_Children(value : ArrayCollection) : ArrayCollection
	{
		super.children = value;
        return value;
    }
	
	override private function get_Children() : ArrayCollection
	{
		return _children;
    }
}