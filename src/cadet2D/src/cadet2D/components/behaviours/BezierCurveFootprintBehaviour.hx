...
class BezierCurveFootprintBehaviour extends Component implements IFootprint
{
	...
	private var _capEnds : Bool = false;
	private var thicknessSquared : Float;
	private var _x : Int;
	private var _y : Int;
    ...
    {
        super();
		name = "BezierCurveFootprintBehaviour";thickness = 20;
    }
	...
	private function validateValues() : Void
	{
		...
		var segments : Array<Dynamic> = QuadraticBezierUtil.clone(_curve.segments);
		...
		for (sizeX) { 
			...
			for (sizeY) {
				...
				if (_capEnds) {
					var base : Bool = VertexUtil.isLeft(v1E, v2E, p.x, p.y) > 0;
					var left : Bool = VertexUtil.isLeft(v2E, v4E, p.x, p.y) > 0;
					var right : Bool = VertexUtil.isLeft(v3E, v1E, p.x, p.y) > 0;
					var tip : Bool = VertexUtil.isLeft(v4E, v3E, p.x, p.y) > 0;
					if (base && left && right && tip) {
						continue;
                    }
				...
				var closestRatio : Float = QuadraticBezierUtil.getClosestRatio(_curve.segments, p.x, p.y);
	...
	private function set_Curve(value : BezierCurve) : BezierCurve
	{
		if (_curve != null) {
	...
	private function get_Curve() : BezierCurve
	{
		return _curve;
    }
	...
	@:meta(Serializable())
	@:meta(Inspectable())
	private function set_CapEnds(value : Bool) : Bool
	{
		_capEnds = value;
		invalidate(VALUES);
        return value;
	...
	private function get_CapEnds() : Bool
	{
		return _capEnds;
    }
	
	private function invalidateCurveHandler(event : ValidationEvent) : Void
	{
		invalidate(VALUES);
    }
