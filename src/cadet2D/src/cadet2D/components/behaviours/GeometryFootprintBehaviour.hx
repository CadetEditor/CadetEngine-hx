...
class GeometryFootprintBehaviour extends Component implements IFootprint
{
	...
	private function validateValues() : Void
	{
		...
		_footprintManager.removeFootprint(this);
		if (Std.is(_geometry, PolygonGeometry)) {
			generatePolygonFootprint(cast((geometry), PolygonGeometry));
        } else if (Std.is(_geometry, CircleGeometry)) {
			generateCircleFootprint(cast((_geometry), CircleGeometry));
        }
	...
	private function generateCircleFootprint(circleGeometry : CircleGeometry) : Void
	{
		...
		var r : Float = circleGeometry.radius;
		var r2 : Float = r * r;
		...
		for (x in 0...sizeX) {
			_values[x] = [];
			for (y in 0...sizeY) {
				var px : Float = (_x * gridSize) + (x * gridSize) + gridSize2;
				var py : Float = (_y * gridSize) + (y * gridSize) + gridSize2;
				var dx : Float = px - position.x;
				var dy : Float = py - position.y;
				var d : Float = dx * dx + dy * dy;_values[x][y] = d < r2;
            }
        }
    }
	...
	private function generatePolygonFootprint(polygon : PolygonGeometry) : Void
	{
		...
		polygon = cast((_geometry), PolygonGeometry);
		var transformedVertices : Array<Dynamic> = VertexUtil.copy(polygon.vertices);
		...
		var originalSquareVertices : Array<Dynamic> = rectangle.vertices;
		for (sizeX) {
			_values[x] = [];
			for (sizeY) {
				var squareVertices : Array<Dynamic> = VertexUtil.copy(originalSquareVertices);
				m.tx = (_x * gridSize) + (x * gridSize); 
				m.ty = (_y * gridSize) + (y * gridSize);
				VertexUtil.transform(squareVertices, m);
				var intersections : Array<Dynamic> = VertexUtil.getIntersections(squareVertices, transformedVertices);
