...
import cadet.core.Component;import cadet.events.ValidationEvent;import cadet2d.components.processes.FootprintManagerProcess;import cadet2d.components.transforms.Transform2D;class SimpleFootprintBehaviour extends Component implements IFootprint
{
...
private var _x : Int;private var _y : Int;private var _sizeX : Int = 1;private var _sizeY : Int = 1;private var _values : Array<Dynamic>;private var _footprintManager : FootprintManagerProcess;private var _transform : Transform2D;public function new()
    {
        super();name = "SimpleFootprintBehaviour";
    }override private function addedToScene() : Void{addSiblingReference(Transform2D, "transform");addSceneReference(FootprintManagerProcess, "footprintManager");
    ...
    }private function validateValues() : Void{if (_transform == null)             return;if (_footprintManager == null)             return;_x = _transform.x;_y = _transform.y;_footprintManager.removeFootprint(this);_values = [];for (x in 0...sizeX){_values[x] = [];for (y in 0...sizeY){_values[x][y] = true;
            }
...
@:meta(Inspectable())
private function set_SizeX(value : Int) : Int{if (value == _sizeX)             return;_sizeX = value;invalidate("values");
        return value;
    }private function get_SizeX() : Int{return _sizeX;
    }@:meta(Serializable())
@:meta(Inspectable())
private function set_SizeY(value : Int) : Int{if (value == _sizeY)             return;_sizeY = value;invalidate("values");
        return value;
